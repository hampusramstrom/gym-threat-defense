...
mean reward for the last 100 episodes as well as printing the Q-table at the
    ...
    with open('q_learning_alpha_0.1_gamma_0.7_eps_0.0001_res.csv', 'w') as f:
      writer = csv.writer(f, delimiter='\t')
      episode_numbers = ['E'] + range(1, num_episodes + 1)
      writer.writerows(zip(episode_numbers, ['A'] + reward_averages))

    with open('q_learning_alpha_0.1_gamma_0.7_eps_0.0001_time.csv', 'w') as f:
      writer = csv.writer(f, delimiter='\t')
      episode_numbers = ['E'] + range(1, num_episodes + 1)
      writer.writerows(zip(episode_numbers, ['A'] + time_averages))

    with open('q_learning_alpha_0.1_gamma_0.7_eps_0.0001_res_std_high.csv', 'w') as f:
        writer = csv.writer(f, delimiter='\t')
        episode_numbers = ['E'] + range(1, num_episodes + 1)
    ...
    with open('q_learning_alpha_0.1_gamma_0.7_eps_0.0001_res_std_low.csv', 'w') as f:
        writer = csv.writer(f, delimiter='\t')
        episode_numbers = ['E'] + range(1, num_episodes + 1)
        stds_up = map(lambda x: x[0] - x[1], zip(reward_averages, reward_stds))
    ...
    with open('q_learning_alpha_0.1_gamma_0.7_eps_0.0001_time_std_high.csv', 'w') as f:
        writer = csv.writer(f, delimiter='\t')
        episode_numbers = ['E'] + range(1, num_episodes + 1)
    ...
    with open('q_learning_alpha_0.1_gamma_0.7_eps_0.0001_time_std_low.csv', 'w') as f:
        writer = csv.writer(f, delimiter='\t')
        episode_numbers = ['E'] + range(1, num_episodes + 1)
        stds_up = map(lambda x: x[0] - x[1], zip(time_averages, time_stds))
